...
import 'package:ibuild_dash/views/components/checkbox.dart';
import 'package:ibuild_dash/views/components/photo_round.dart';
import 'package:ibuild_dash/views/widgets/modal_partner.dart';

class ParceirosAtivosScreen extends StatefulWidget {
  const ParceirosAtivosScreen({Key? key}) : super(key: key);

  @override
  State<ParceirosAtivosScreen> createState() => _ParceirosAtivosScreenState();
}

class _ParceirosAtivosScreenState extends State<ParceirosAtivosScreen> {
  String search = "";
  bool somenteAssinantes = false;
  ...
  filter() {
    var tmp = dashboardController.partners;
    ...
    return tmp
        .where((e) => !somenteAssinantes ? true : e.subscribed)
        .where((e) => e.companyName.search(search))
        .toList();
  }
